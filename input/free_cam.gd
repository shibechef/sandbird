...
func move_camera(delta: float) -> void:
	...
	var cos: float = cos(deg_to_rad(camera.rotation_degrees.y))
	var sin: float = sin(deg_to_rad(camera.rotation_degrees.y))
	...
	camera.position += velocity * speed_multiplier * delta
